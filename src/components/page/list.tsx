    ...
    >
      ...
      >
        ...
        >
          ...
          <p data-testid="ListTitle" className="line-clamp-1 flex-1 text-xl">{ constant.base.app.name }</p>
        ...
        <ul data-id="ListBody" data-testid="ListBody" className="flex-1 select-none overflow-auto p-3">
          { props.list.state.data.sort.map((listId) =>
            <li
              ...
              className={`${props.list.render.classStatus({listId, selectId: params.listId!})} ${props.list.render.classLimit({listId})} theme-color-border theme-color-back trans-select-label trans-edit-item anime-scale-item group relative flex h-16 items-center gap-3 overflow-hidden border-b-[0.1rem] border-solid p-3`}
            ...
            >
              ...
              <div className="theme-color-back trans-option-label absolute right-3 flex translate-x-[150%] gap-3">
                ...
                { listId !== params.listId && listId !== constant.base.id.trash &&
                  <IconTrash
