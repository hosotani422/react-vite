  ...
  return (
    <BasePopup data-testid="ClockRoot" open={props.clock.state.open} max={true}>
      ...
      <div className="flex flex-initial items-center justify-end gap-3">
        ...
        >
        <InputButton data-testid="ClockClear" className="flex-initial text-theme-warn" onClick={() => props.clock.temp.callback()}>
          { props.clock.state.clear }</InputButton
        >
        <InputButton
          data-testid="ClockOk"
          className="flex-initial text-theme-warn"
          onClick={() => props.clock.temp.callback({ hour: props.clock.state.hour, minute: props.clock.state.minute })}
        >
